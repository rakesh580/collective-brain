...
export default function OnboardingWizard({ onDismiss }: Props) {
 ...
 return (
 <div className="bg-elevated border border-default rounded-2xl p-6 mb-6 shadow-sm relative overflow-hidden">
 ...
 <button
 onClick={onDismiss}
 className="absolute top-3 right-3 text-slate-400 transition-colors z-10" aria-label="Dismiss onboarding wizard" >
 <X size={16} />
 </button>
 ...
 i <= currentStep
 ? "bg-gradient-to-r from-indigo-500 to-violet-500" : "bg-muted" }`}
 />
 ))}
 </div>
 ...
 {/* Step indicators */}
 <div className="flex justify-center gap-4 mt-5 pt-4 border-t border-subtle">
 {steps.map((s, i) => {
 ...
 i === currentStep
 ? "text-indigo-600 font-medium" : i < currentStep
