    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="share-modal-title" className="relative bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl p-6 w-full max-w-sm">
        ...
        {participants.length > 0 && (
          <div className="mb-4">
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase mb-2">Shared with</p>
            <div className="space-y-1">
              {participants.map((p) => (
                <div key={p.user_id} className="flex items-center justify-between bg-slate-50 dark:bg-slate-900/50 rounded-lg px-3 py-2">
        ...
        {availableUsers.length > 0 ? (
          <>
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase mb-2">Add people</p>
            <div className="space-y-1 max-h-40 overflow-auto mb-4">
              {availableUsers.map((u) => (
                ...
                >
                  ...
                  />
                  <span className="text-sm text-slate-700 dark:text-slate-300">{u.display_name || u.username}</span>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">No more users to add.</p>
        )}
