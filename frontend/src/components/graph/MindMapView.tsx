      ...
      <div className="absolute top-4 left-4 z-20 flex flex-col gap-2">
        <div className="relative">
          ...
          {searchMatches.size > 0 && (
            <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-2xs font-bold text-violet-600 dark:text-violet-400">
        ...
        <div className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm rounded-xl p-3 border border-slate-200 dark:border-slate-700 shadow-lg">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-2xs text-slate-500 dark:text-slate-400">Collab</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-5 h-0 border-t-2" style={{ borderColor: "#34d399" }} />
              <span className="text-2xs text-slate-500 dark:text-slate-400">Expertise</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute top-4 right-4 z-20 flex flex-col gap-2">
        ...
        <div className="flex flex-col bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm rounded-xl border border-slate-200 dark:border-slate-700 shadow-lg overflow-hidden">
          <button
            ...
            className={`px-3 py-2 text-2xs font-semibold transition-colors ${layoutMode === "radial" ? "bg-violet-100 dark:bg-violet-500/20 text-violet-700 dark:text-violet-300" : "text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700"}`}
          >
            Radial
          </button>
          <div className="border-t border-slate-100 dark:border-slate-700" />
          <button
            ...
            className={`px-3 py-2 text-2xs font-semibold transition-colors ${layoutMode === "tree" ? "bg-violet-100 dark:bg-violet-500/20 text-violet-700 dark:text-violet-300" : "text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700"}`}
          >
            Tree
          </button>
        </div>
        ...
        {/* Zoom % */}
        <div className="text-center text-2xs font-mono font-bold text-slate-400 dark:text-slate-500">
          {Math.round(zoom * 100)}%
        </div>
      </div>
              ...
              {Array.isArray(value) ? (
                <div className="flex gap-1 flex-wrap mt-0.5">
                  {(value as string[]).slice(0, 8).map((v) => (
                    <span key={v} className="text-2xs bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-1.5 py-0.5 rounded">{v}</span>
                  ))}
                </div>
              ) : (
