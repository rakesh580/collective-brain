...
export default function HeatmapView() {
  ...
  const { sortedMembers, activeTopics, memberTotals, topicAverages, busFactor, gapTopics, strongTopics } = useMemo(() => {
    if (!data) return { sortedMembers: [], activeTopics: [], memberTotals: new Map(), topicAverages: new Map(), busFactor: [], gapTopics: [], strongTopics: [] };
  ...
  return (
    <div className="p-4 sm:p-6 h-full overflow-auto">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-7 pr-3 py-1.5 w-40 text-xs bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-700 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/30"
            />
          </div>
          ...
          <button
            ...
            className={`p-1.5 rounded-lg border transition-colors ${showFilterPanel ? "bg-indigo-50 dark:bg-indigo-500/20 border-indigo-200 dark:border-indigo-700 text-indigo-600 dark:text-indigo-400" : "border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700"}`}
            title="Filter topics"
          >
          ...
          <button
            onClick={exportCSV}
            className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
            title="Export CSV"
          >
          ...
          <button
            ...
            className={`p-1.5 rounded-lg border transition-colors ${showGaps ? "bg-amber-50 dark:bg-amber-500/10 border-amber-200 dark:border-amber-700 text-amber-600 dark:text-amber-400" : "border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700"}`}
      ...
      {/* ── Filter panel ── */}
      {showFilterPanel && (
        <div className="mb-4 p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm">
      ...
      {showGaps && (busFactor.length > 0 || gapTopics.length > 0 || strongTopics.length > 0) && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
          {/* Bus Factor Risks */}
          {busFactor.length > 0 && (
            <div className="p-3 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-800/30 rounded-xl">
          ...
          {/* Knowledge Gaps */}
          {gapTopics.length > 0 && (
            <div className="p-3 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-800/30 rounded-xl">
              <div className="flex items-center gap-1.5 mb-2">
          ...
          {strongTopics.length > 0 && (
            <div className="p-3 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-800/30 rounded-xl">
              <div className="flex items-center gap-1.5 mb-2">
      ...
      <div className="overflow-auto border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm">
        <table className="min-w-full border-collapse">
          <thead>
            <tr>
              <th className="sticky left-0 z-20 bg-white dark:bg-slate-800 px-3 py-3 text-left text-xs font-bold text-slate-600 dark:text-slate-300 border-b border-r border-slate-200 dark:border-slate-700 min-w-[180px]">
                <button onClick={() => handleSort("name")} className="flex items-center gap-1 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                  Member
              ...
              {activeTopics.map((topic) => {
                ...
                return (
                  <th
                    key={topic}
                    className={`px-1 py-2 text-center border-b border-slate-200 dark:border-slate-700 min-w-[68px] transition-colors cursor-pointer group ${
                      hoveredCol === topic ? "bg-indigo-50 dark:bg-indigo-500/10" : ""
                    } ${isSorted ? "bg-violet-50 dark:bg-violet-500/10" : ""}`}
                  ...
                  >
                    <div className="flex flex-col items-center gap-0.5 relative">
                      ...
                      {/* Sort icon on hover */}
                      {!isSorted && (
                        <ArrowUpDown className="w-2.5 h-2.5 text-slate-300 dark:text-slate-600 opacity-0 group-hover:opacity-100 transition-opacity mt-0.5" />
                      )}
                    </div>
                  </th>
                );
          ...
          <tbody>
            {sortedMembers.map((member, idx) => {
              ...
              return (
                <tr
                  key={member.id}
                  className={`transition-colors ${isRowHovered ? "bg-slate-50 dark:bg-slate-800/60" : idx % 2 === 0 ? "" : "bg-slate-50/30 dark:bg-slate-800/20"}`}
                  onMouseEnter={() => setHoveredRow(member.id)}
                  onMouseLeave={() => setHoveredRow(null)}
                >
                  <td className={`sticky left-0 z-10 px-3 py-2 border-r border-slate-200 dark:border-slate-700 ${isRowHovered ? "bg-slate-50 dark:bg-slate-800/60" : "bg-white dark:bg-slate-800"}`}>
                    <button
                  ...
                  {activeTopics.map((topic) => {
                    ...
                    return (
                      <td
                        ...
                        onMouseEnter={() => { setHoveredCell({ member: member.id, topic }); setHoveredCol(topic); }}
                  ...
                  {/* Sparkline */}
                  <td className="px-2 py-2 border-l border-slate-200 dark:border-slate-700 text-center">
                    <Sparkline values={sparkValues} />
                  </td>
                  <td className={`sticky right-0 z-10 px-3 py-2 border-l border-slate-200 dark:border-slate-700 text-center ${isRowHovered ? "bg-slate-50 dark:bg-slate-800/60" : "bg-white dark:bg-slate-800"}`}>
            ...
            <tr className="border-t-2 border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-800">
              ...
              <td className="px-2 py-2 border-l border-slate-200 dark:border-slate-700" />
