...
async def search(request: Request, q: str, room_id: str | None = None, limit: int = 20, semantic: bool = False):
    ...
    try:
        ...
        member_query = db.query(MemberRecord).filter(
            MemberRecord.name.ilike(pattern)
        ).limit(limit)
...
def _perform_semantic_search(request: Request, query: str, n_results: int = 20, room_id: str | None = None) -> list[dict]:
