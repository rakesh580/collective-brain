...
class TestMarkdownUpload:
    ...
    def test_upload_requires_auth(self, app_client):
        ...
        resp = app_client.post("/api/v1/ingest/markdown-upload", files=files)
...
class TestDocumentUpload:
    def test_upload_txt_file(self, app_client, auth_headers):
        ...
        resp = app_client.post("/api/v1/ingest/documents", headers=auth_headers, files=files)
        assert resp.status_code == 200
...
class TestSlackIngestion:
    ...
    def test_ingest_slack_requires_auth(self, app_client):
        ...
        buf.seek(0)
        resp = app_client.post("/api/v1/ingest/slack", files=[("file", ("e.zip", buf, "application/zip"))])
        assert resp.status_code == 401
...
class TestDiscordIngestion:
    def test_ingest_discord_json(self, app_client, auth_headers):
        ...
        resp = app_client.post("/api/v1/ingest/discord", headers=auth_headers, files=files)
        assert resp.status_code == 200
...
class TestTaskIngestion:
    def test_ingest_tasks_json(self, app_client, auth_headers):
        ...
        resp = app_client.post("/api/v1/ingest/tasks", headers=auth_headers, files=files)
        assert resp.status_code == 200
