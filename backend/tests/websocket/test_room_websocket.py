...
class TestRoomCreation:
    ...
    def test_list_rooms(self, app_client, auth_headers):
        app_client.post("/api/v1/rooms", headers=auth_headers, json={"name": "room-a"})
        app_client.post("/api/v1/rooms", headers=auth_headers, json={"name": "room-b"})
        resp = app_client.get("/api/v1/rooms", headers=auth_headers)
        assert resp.status_code == 200
        assert len(resp.json()["rooms"]) >= 2

    def test_create_room_requires_auth(self, app_client):
        resp = app_client.post("/api/v1/rooms", json={"name": "unauth"})
...
class TestRoomMembers:
    def test_add_member_to_room(self, app_client, auth_headers, registered_user):
        ...
        # Create room
        room = app_client.post("/api/v1/rooms", headers=auth_headers, json={"name": "team"})
    ...
    def test_leave_room(self, app_client, auth_headers, registered_user):
        room = app_client.post("/api/v1/rooms", headers=auth_headers, json={"name": "temp-room"})
        room_id = room.json()["id"]
        user_id = registered_user[0]["id"]

        resp = app_client.delete(f"/api/v1/rooms/{room_id}/members/{user_id}", headers=auth_headers)
        assert resp.status_code == 200


class TestRoomMessages:
    def test_send_message(self, app_client, auth_headers):
        room = app_client.post("/api/v1/rooms", headers=auth_headers, json={"name": "msg-room"})
        room_id = room.json()["id"]
    ...
    def test_list_messages(self, app_client, auth_headers):
        room = app_client.post("/api/v1/rooms", headers=auth_headers, json={"name": "hist-room"})
        room_id = room.json()["id"]
        ...
        resp = app_client.get(f"/api/v1/rooms/{room_id}/messages", headers=auth_headers)
        assert resp.status_code == 200
    ...
    def test_send_message_requires_membership(self, app_client, auth_headers):
        ...
        room = app_client.post("/api/v1/rooms", headers=auth_headers, json={"name": "private-room"})
        room_id = room.json()["id"]
...
class TestRoomWebSocket:
    def test_websocket_auth_required(self, app_client):
        ...
        room_resp = app_client.post("/api/v1/rooms", headers=headers, json={"name": "ws-room"})
        room_id = room_resp.json()["id"]

        with app_client.websocket_connect(f"/api/v1/rooms/ws/{room_id}") as ws:
            # Send invalid token
    ...
    def test_websocket_message_broadcast(self, app_client):
        ...
        room = app_client.post("/api/v1/rooms", headers=headers, json={"name": "ws-msg-room"})
        room_id = room.json()["id"]
    ...
    def test_websocket_typing_indicator(self, app_client):
        ...
        room = app_client.post("/api/v1/rooms", headers=headers, json={"name": "ws-type-room"})
        room_id = room.json()["id"]
    ...
    def test_websocket_ping_pong(self, app_client):
        ...
        room = app_client.post("/api/v1/rooms", headers=headers, json={"name": "ws-ping-room"})
        room_id = room.json()["id"]
...
class TestRoomAIQuery:
    def test_ai_query_in_room(self, app_client, auth_headers):
        room = app_client.post("/api/v1/rooms", headers=auth_headers, json={"name": "ai-room"})
        room_id = room.json()["id"]

        resp = app_client.post(
            f"/api/v1/rooms/{room_id}/ai",
            headers=auth_headers,
